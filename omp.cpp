...
void vector_flater_omp(const vector<double>* vec, const vector<int>* fixed_indexes, vector<double>* flat) {
  ...
  for (size_t i = 1; i < vec->size() - 1; i++) {
    ...
    if (!skip) {
      ...
      // std::cout << vec->at(i - 1) << " + " << vec->at(i + 1) <<" index" << i << " avg: " << avg << std::endl;
      (*flat)[i] = avg;
    }
  }
}
